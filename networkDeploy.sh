...
docker kill cliCustomers cliRegulators logspout || true
./network.sh down
./network.sh up createChannel -ca -s couchdb

# Copy the connection profiles so they are in the correct organizations.
cp "${DIR}/../test-network/organizations/peerOrganizations/org1.example.com/connection-org1.yaml" "${DIR}/customers/configuration/gateway/"
cp "${DIR}/../test-network/organizations/peerOrganizations/org2.example.com/connection-org2.yaml" "${DIR}/regulators/configuration/gateway/"
...
echo "./stakeholders/customers/configuration/cli/monitordocker.sh fabric_test"
