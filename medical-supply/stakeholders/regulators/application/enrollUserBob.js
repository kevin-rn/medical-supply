...
async function main() {
    try {
        ...
        const walletPath = path.join(process.cwd(), '../identity/user/bob/wallet');
        const wallet = await Wallets.newFileSystemWallet(walletPath);
        console.log(`Wallet path: ${walletPath}`);

        // Check to see if we've already enrolled the admin user.
        const userExists = await wallet.get('bob');
        if (userExists) {
        ...
        await wallet.put('bob', x509Identity);
        console.log('Successfully enrolled client user "bob" and imported it into the wallet');

    } catch (error) {
        console.error(`Failed to enroll client user "bob": ${error}`);
        process.exit(1);
    }
}
